		...
		for(int i = rowStart; i < rowEnd; i++){
			for(int j = colStart; j < colEnd; j++){
				if(board[i][j] != eChar.BLANK){
					...
					case STEWARD:
						charPlace = new DragComponent("imgs/pika.png",quad, board[i][j],j%38*width, i%24*height);
						
						pane.add(charPlace, 0);
						break;
					case RESEARCHER:
						charPlace = new DragComponent("imgs/oak.png",quad, board[i][j], j%38*width, i%24*height);
						
						pane.add(charPlace, 0);
						break;
					case VOLUNTEER:
						charPlace = new DragComponent("imgs/red.png",quad, board[i][j],j%38*width, i%24*height);
